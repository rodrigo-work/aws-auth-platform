...
export async function getUser(): Promise<any> {
  ...
  try {
    ...
    const parsedValues = {
      ...
      mfa: result.UserMFASettingList?.includes('SOFTWARE_TOKEN_MFA') ?? false
    }
    ...
    return { title: 'Failed to get user:', error: `${(error as Error).message}` }
  }
}
...
export async function updateUser({ attributes }: any) {
  ...
  //     limiter: slidingWindow(3, '1 d')
  //   })
  //   const head = await headers()
  //   const ip = head.get('x-forwarded-for')

  //   const { success } = await rateLimiter.limit(`${'title'}-${ip}`)
